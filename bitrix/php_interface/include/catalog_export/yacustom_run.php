	...
	if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE'])
	{
		...
		while ($obElement = $res->GetNextElement())
		{
			...
			$rsProducts = CCatalogProduct::GetList(
				...
				false,
				false,
				array('ID', 'QUANTITY', 'QUANTITY_TRACE', 'CAN_BUY_ZERO', 'PURCHASING_PRICE')
			);
			if ($arProduct = $rsProducts->Fetch())
			{
				$arProduct['QUANTITY'] = doubleval($arProduct['QUANTITY']);
				if (0 >= $arProduct['QUANTITY'])
...
/*					$dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
			...
			// Исправлено условие ($minPrice <= 0) в рамках задачи от 2021.09.19
			if($minPrice <= 0) continue;
			//if ($minPrice < 1000) continue;
			
			// перечень разделов, из которых нужно выбрать товары с ценой менее 1000 р.
			// UPD 2021.10.01 больше не нужно включать в фид товары с ценой менее 1000 р.
			/*$arSectionsLessThan1000 = array(4938, 5148, 4992, 4940, 5138, 4832, 4833, 4972, 5007, 5169, 5207, 5099, 5074, 5109,
											5158, 5142, 5167, 5111, 5085, 5159, 4979, 5150, 4840, 4882, 4999, 5066, 5027, 5147,
											4915, 5102, 4877, 5118, 5028, 5126, 4903, 5152, 5127, 4848, 4924, 4890, 4926, 5033,
											5006, 4973, 5098, 4954, 5165, 5018, 4885, 5051, 5191, 5188, 5193, 5190, 5192, 5189,
											4995, 5034, 4866, 5211, 4835, 5029, 4994, 5129, 5086, 5013, 4956, 4931, 5056, 5024,
											4892, 4913, 4982, 5036, 4828, 4896, 5091, 4829, 5121, 4920, 5123, 5206, 5208, 5022,
											4918, 5067, 5017, 4989, 4922, 4875, 5136, 5101, 5157, 4955, 4893, 5105, 4977, 4968,
											4827, 5151, 4902, 4868, 5145, 4879, 4985, 5153, 5114, 5055, 5001, 4991, 4935, 5166,
											4943, 4964, 5083, 4881, 4939, 4960, 5095, 4850, 4925, 4878, 4862, 5141, 4863, 5124,
											5143, 5164, 4894, 5004, 5149, 5170, 4854, 5003, 4980, 4763, 4770, 4771
											);
			*/
			...
			$bNoActiveGroup = true;
			$strTmpOff_tmp = "";
			$db_res1 = CIBlockElement::GetElementGroups($arAcc["ID"], false, array('ID', 'ADDITIONAL_PROPERTY_ID', 'CODE'));
			$section_code = '';
			while ($ar_res1 = $db_res1->Fetch())
			{
				...
				// задача от 2021.09.20
				// в выгрузку должны попадать товары с ценой больше 1000 р, а также все товары из определенного списка разделов (см. выше)
				// UPD 2021.10.01 товары ниже 1000 рублей невыгодно размещать, отменяем список разделов. Оставляем только отсечку по цене (более 1000 р.)
				if($minPrice <= 1000) continue;
				// if(($minPrice <= 1000) && (!in_array((int)$ar_res1["ID"], $arSectionsLessThan1000))) continue;
				// конец задачи от 2021.09.20	
				
				if (in_array((int)$ar_res1["ID"], $arSectionIDs))
				{
					$strTmpOff_tmp.= "<categoryId>".$ar_res1["ID"]."</categoryId>\n";
					$bNoActiveGroup = false;
					$section_code = $ar_res1["CODE"];
				}
			}
			...
			$strTmpOff.= "<url>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arAcc["~DETAIL_PAGE_URL"]).(strstr($arAcc['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."r1=".$strReferer1."&amp;r2=".$strReferer2."&amp;utm_source=yandex_market&amp;utm_medium=cpc&amp;utm_campaign=".$section_code."&amp;utm_term=".$arAcc["ID"]."</url>\n";

            /*
             * Убираем решение вытаскивание цены из
            if($arAcc['PROPERTIES']['PRICE_MARKET']['VALUE'])
			    $strTmpOff.= "<price>".$arAcc['PROPERTIES']['PRICE_MARKET']['VALUE']."</price>\n";
            else
                $strTmpOff.= "<price>".$minPrice."</price>\n";
            */

            //Делаем просто на 15% дороже обычной цены
            $strTmpOff.= "<price>".ceil($minPrice*1.15)."</price>\n";

			if((isset($arProduct["PURCHASING_PRICE"])) AND ($arProduct["PURCHASING_PRICE"] > 0)){
				$strTmpOff.= "<purchase_price>".ceil($arProduct["PURCHASING_PRICE"])."</purchase_price>\n";
			}

			$strTmpOff.= "<currencyId>".$minPriceCurrency."</currencyId>\n";
			
			// добавление обязательного поля vendor
			$strTmpOff.= "<vendor>".$vendorNoValueYet."</vendor>\n";
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
                    ...
                    if($arOfferItem['PROPERTIES']['PRICE_MARKET']['VALUE'])
					    $strOfferYandex .= "<price>".$arOfferItem['PROPERTIES']['PRICE_MARKET']['VALUE']."</price>\n";
                    else
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
	...
	// Доработка для того, чтобы файл с выгрузкой был доступен практически постоянно
	$BufferArr = explode("/",$SETUP_FILE_NAME);
	$BufferArr[count($BufferArr)-1] = "YandexCustomResult.php";
	$YandexResultFile = implode("/",$BufferArr);
	
	copy($_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME, $_SERVER["DOCUMENT_ROOT"].$YandexResultFile);
	
	//file_put_contents(__DIR__ . "/logs/yandex.log",json_encode(array_slice($GLOBALS, 1))."\n",FILE_APPEND);
}
